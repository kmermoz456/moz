@php
    $initialQuestions = isset($quiz)
        ? $quiz->questions->map(fn ($q) => ['question' => $q->question, 'choix' => $q->choix, 'bonne_reponse' => $q->bonne_reponse])->values()->all()
...
<div x-data="{ questions: {{ Illuminate\Support\Js::from($initialQuestions) }} }">
    <template x-for="(q, qi) in questions" :key="qi">
        <div class="border border-gray-200 rounded-xl p-5 mb-4">
            ...
            <p class="text-xs text-gray-500 mt-2">Cochez le bouton radio du choix qui est la bonne réponse.</p>
        </div>
    </template>

    <button type="button" @click="questions.push({ question: '', choix: ['', ''], bonne_reponse: '' })"
