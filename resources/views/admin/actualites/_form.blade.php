...
<div class="space-y-5">
    ...
    <div>
        <label class="block font-semibold text-itf-dark mb-1">Contenu</label>
        <textarea name="contenu" rows="5" required
                  class="w-full rounded-lg border-gray-300 focus:border-itf-blue focus:ring-itf-blue">{{ old('contenu', $actualite->contenu ?? '') }}</textarea>
    </div>

    <div>
        <label class="block font-semibold text-itf-dark mb-1">
            Image @if (isset($actualite)) (laisser vide pour conserver l'actuelle) @endif
        </label>
    ...
    <label class="flex items-center gap-2">
        <input type="checkbox" name="publie" value="1" @checked(old('publie', $actualite->publie ?? true))
               class="rounded border-gray-300 text-itf-blue focus:ring-itf-blue">
        <span class="text-itf-dark">Publiée sur le site</span>
    </label>
